...
#   )
   ...
   if lensinfo.size <= 6:
      ...
      plt.figure(figsize=(nband*panelsize,6*panelsize))
      xsize = 1.0/nband - 0.005
      ysize = 1.0/6.- 0.005
   ycount = 1
   ...
   for i in range(lensinfo.size):
      ...
      print ''
      if i == 6:
         plt.figure(figsize=(nband*panelsize,(lensinfo.size-6)*panelsize))
         xsize = 1.0/nband - 0.005
         ysize = 1.0/(lensinfo.size-6) - 0.005
      for j in range(nband):
         ...
         if lensinfo.size <= 6:
            ...
            ystart = 1.0025 - 1.0 * ycount / 6
         else:
            ystart = 1.0025 - 1.0 * (ycount-6) / (lensinfo.size-6)
         if (j % nband) == nband-1:
            ycount += 1
         ...
         plt.axis('off')
         del f
      if ((i == 5) & (lensinfo.size > 6)):
         if outfile != None:
            if outfile[len(outfile)-2:] == 'ps':
               newoutfile = outfile[:len(outfile)-2] + '_1.ps'
            elif outfile[len(outfile)-3:] == 'png':
               newoutfile = outfile[:len(outfile)-2] + '_1.png'
            else:
               sys.exit('Unknown file extension used (I only wrote code for ps and png)')
            plt.savefig(newoutfile)
   ...
   if outfile != None: 
      if lensinfo.size <= 6:
         ...
         if outfile[len(outfile)-2:] == 'ps':
            newoutfile = outfile[:len(outfile)-2] + '_2.ps'
         elif outfile[len(outfile)-3:] == 'png':
            newoutfile = outfile[:len(outfile)-2] + '_2.png'
         else:
            sys.exit('Unknown file extension used (I only wrote code for ps and png)')
         plt.savefig(newoutfile)
