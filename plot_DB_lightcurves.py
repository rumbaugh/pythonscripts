...
def plot_lightcurve(dbid,mjd,mag,magerr,bands,survey,trueredshift,DBdir,psfpage=None,fname=None,DESfname=None,connectpoints=True,specfile=None,WavLL=3000,WavUL=10500,outlierflag=0,zoominband=None,outlierarr=None):
    ...
    for b in ['g']:
        if np.shape(outlierarr)!=():
            gb=np.where((outlierarr==0)&(bands==b)&(survey!='POSS'))[0]
        else:
    ...
    if not(DESfname in [None,'False']):
        ...
        ax4.imshow(img4)
        ax4.plot(np.array([0.5,0.5]),np.array([0.55,0.8]),color='white',transform=ax4.transAxes)
        ax4.plot(np.array([0.5,0.5]),np.array([0.45,0.2]),color='white',transform=ax4.transAxes)
        ax4.plot(np.array([0.45,0.2]),np.array([0.5,0.5]),color='white',transform=ax4.transAxes)
        ax4.plot(np.array([0.55,0.8]),np.array([0.5,0.5]),color='white',transform=ax4.transAxes)
    if len(gsdss)>0:
